...
#Deploy Application
 ...
 envsubst < ${APP_MANIFEST_DIR}/flaskapp1.yaml | kubectl apply -f -
 envsubst < ${APP_MANIFEST_DIR}/Service_Admin.yaml | kubectl apply -f -
 envsubst < ${APP_MANIFEST_DIR}/Service_User.yaml | kubectl apply -f -
 envsubst < ${APP_MANIFEST_DIR}/Deployment_Admin.yaml | kubectl apply -f -
 envsubst < ${APP_MANIFEST_DIR}/Deployment_User.yaml | kubectl apply -f -

#Step 3.3: Verify that your worker nodes registered with your cluster
#Step 3.4: Launch a sample application
#Step 3.5: Clean up

#Generate KUBE_CONFIG_DATA for Github Secret

cat $HOME/.kube/config | base64 > ~/.private/kubeconfig.base64